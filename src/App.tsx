  ...
  return (
    <div style={{ height: '100%' }}>
      <Fullscreen enabled={isFull} onChange={(isFull) => setIsFull(isFull)}>
        ...
        />
        <Button size="massive" color="blue" circular icon="play" onClick={props.login}></Button>
...
const App: React.FC = () => {
  ...
  const login = () => {
    logger.info('--------------------------------------------------');
    logger.info('-- status value here', status.status);
    logger.info('-- streamer status value here', streamerStatus);
    logger.info('--------------------------------------------------');

    if (status.status === 'ready' && streamerStatus === 'Connected') {
      const descriptor = {
        type: 'login',
        message: {
          payload: {}
